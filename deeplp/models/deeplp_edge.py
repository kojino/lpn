...
class DeepLP_Edge(DeepLP_WRBF):
    ...
    def _init_weights(self, features, graph, seed):
        ...
        if seed==0:
            ...
            np.random.seed(seed)
            theta = np.random.normal(loc=0.0,scale=4.0,size=(1,num_features))
        ...
        self.weights_unnormalized = tf.SparseTensor(nonzero_indices, self.nonzero_values, [num_nodes, num_nodes])
