...
def main(args):
    ...
    graph, weights = load_data(args.data)
    ...
    def save_result(results,unlabeled_pred,model,weight_id,iter_id,clamp,laplacian):
        # print('-------------------')
        ...
        results = results.append({'data': args.data,
                                  'model': model,
    ...
    for weight_id,weights_np in enumerate([weights, weights_one]):
        unlabeled_pred = lp.closed_sp(labels,
                                   ...
                                   labeled_indices,
                                   unlabeled_indices)
        results = save_result(results,unlabeled_pred,'LP closed',weight_id,None,None,None)

    print("LP iterative form")
    for iter_id in [10,50,100]:
        print(iter_id)
        for weight_id,weights_np in enumerate([weights, weights_one]):
            print(weight_id)
            ...
            unlabeled_pred = lp.iter_sp(labels,
                                 ...
                                 iter_id,
                                 unlabeled_indices)
            results = save_result(results,unlabeled_pred,'LP Iter',weight_id,iter_id,None,None)
            # print('clamp')
    ...
    if args.data[:5] == 'linqs':
        ...
        print(features[labeled_indices].shape,np.argmax(labels[labeled_indices],axis=1).shape)
        ...
        results = save_result(results,res,'Logistic Regression',None,None,None,None)
    ...
    results = save_result(results,h[unlabeled_indices],'LLGC',None,None,None,None)
    ...
    results = save_result(results,Yhat[unlabeled_indices][:,:-1],'Adsorption',None,None,None,None)
