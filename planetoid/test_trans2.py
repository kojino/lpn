...
x
print 'x:', type(x), x.dtype, x.shape, x
print 'y:', type(y), y.dtype, y.shape, y
print 'tx', type(tx), tx.dtype, tx.shape, tx
print 'ty:', type(ty), ty.dtype, ty.shape, ty
raw_input('press enter')

m = model(args)                                             # initialize the model
m.add_data(x, y, graph)                                     # add data
m.build()                                                   # build the model
m.init_train(init_iter_label = 10, init_iter_graph = 3)  # pre-training
