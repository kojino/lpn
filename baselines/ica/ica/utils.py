...
def pick_aggregator(agg, domain_labels):
    if agg == 'count':
        ...
        raise ValueError('Invalid argument for agg (aggregation operator): ' +
                         str(agg))
    return aggregator
...
def load_data(dataset_str):
    ...
    if dataset_str == 'citeseer':
        ...
        ty_extended_ = np.ones((len(test_idx_range_full),
                                1))  # add dummy labels
...
def load_data_2(data_path, seed, unlabel_prob):
    ...
    print(data_path)
    """Load data."""
    graph = np.loadtxt(data_path + '/Gsym.csv', delimiter=',').astype(int)
    num_nodes = max(max(graph[:, 0]), max(graph[:, 1])) + 1
    adj = csr_matrix(
        (graph[:, 2], (graph[:, 0], graph[:, 1])),
        shape=(num_nodes, num_nodes))

    features = np.loadtxt(data_path + '/x.csv', delimiter=',').astype(int)
